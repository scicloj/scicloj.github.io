...
* `hiccup` - using [Hiccup](https://github.com/weavejester/hiccup)-style markup
...
* `pmap` - parallel version of Clojure's `pmap`
* `for` - parallel version of Clojure's `for`
* `future` - parallel version of Clojure's `future`
* `interop` - general libraries for interop
* `py` - interop with Python
* `r` - interop with R
...
- [Org-babel-clojure](https://orgmode.org/worg/org-contrib/babel/languages/ob-doc-clojure.html) :star:: `lit` - executing Clojure inside Emacs Org-mode documents
